...
export default function History({ hangul, user }) {
  ...
  useEffect(() => {
    const populateHistory = async () => {
      ...
      data.forEach((record) => {
        ...
        let correctCount = 0, halfCount = 0, wrongCount = 0
  ...
  return (
    <>
      <Typography color='primary' sx={{ my: 5 }} variant="h3">{user.username}'s Practice History</Typography>
      <Paper elevation={6} sx={{ display: 'flex', flexDirection: 'row', p: 4, gap: 3, flexWrap: 'wrap' }}>
        {history.map((item) => {
          return (
            <Card elevation={3} sx={{ p:2 }}>
              <Chip label={item.date} sx={{ mb: 2 }} color="secondary" onClick={handleClick}>
...
const getHistory = async (id) => {
  return await fetch(`http://localhost:3001/sessions?userID=${id}`)
    .then(res => {
